...
所有数据库操作（读取和写入）均包含自动重试逻辑：
- 遇到瞬态错误（繁忙、锁定、云同步导致的 I/O 错误）时，操作最多重试 5 次
- 使用指数退避策略（200ms、400ms、800ms、1600ms、3200ms）
- 覆盖 `SQLITE_BUSY`、`SQLITE_LOCKED`、`SQLITE_IOERR_SHORT_READ` 及其他瞬态 I/O 错误
...
以下 SQLite 设置针对云同步场景进行了优化，按连接级别应用以确保连接池中的每个连接都具有正确配置：
